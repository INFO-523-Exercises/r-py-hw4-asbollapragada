...
To illustrate how linear regression works, I'm taking two variable (x and y) from a dataset and plot scatterplot the same.

```{r, warning=FALSE}
# Parameters
numInstances <- 2088  # number of data instances

# Loading CSV file
female_jobs <- read.csv("jobs_gender.csv")

# Assigning X and y variables
X <- female_jobs$total_earnings_female
y <- female_jobs$percent_female
...
# Reassigning X and y after removing NAs
X <- data$X
y <- data$y

# Number of instances after removing NA values
...
ggplot(data = data) +
  ...
  ggtitle('Total earnings of females in different jobs w.r.t. percentage of females') +
  xlab('Total number of females') +
...
Here, Python scikit-learn package will be used to fit a multiple linear regression (MLR) model. Given a training set MLR is designed to learn the regression function by minimizing the following loss function given a training set.

Given the input dataset, steps to be followed are:
...
```{r, warning = FALSE}
# If the lengths are different, subset y_test to match the length of y_pred_test
y_test <- y_test[1:length(y_pred_test)]

# Plotting true vs predicted values after aligning the lengths
ggplot() +
...
ggplot() +
  ...
  ggtitle(sprintf('Predicted Function: y = %.5fX + %.5f', slope, intercept)) +
  xlab('Total number of females') +
  ylab('Percentage of females')
```
...
Here, the presence of correlated attributes can affect the performance of regression models will be illustrated. Specifically, I created 4 additional variables, X2, X3, X4, and X5 that are strongly correlated with the previous variable X. The relationship between X and y remains the same as before. I then fit y against the predictor variables and compare their training and test set errors.

First, I created the correlated attributes below.
...
Below, I trained 4 new regression models based on the 4 versions of training and test data created in the previous step.
...
For postprocessing, I computed both the training and test errors of the models.
...
Ridge regression is a variant of MLR designed to fit a linear model to the dataset by minimizing the regularized least-square loss function

Here, I fit a ridge regression model to the previously created training set with correlated attributes. I compared the results of the ridge regression model against those obtained using MLR.
...
While both ridge and lasso regression methods can potentially alleviate the model overfitting problem, one of the challenges is how to select the appropriate hyperparameter value. Below, I demonstrated examples of using a 5-fold cross-validation method to select the best hyperparameter of the model.
...
I illustrated how to apply cross-validation to select the best hyperparameter value for fitting a lasso regression model.
